...
function dniIsFullyVoted (req, res, next) {
  // check if the req.body.dni has not voted config.cantVotesAvailable times
  apiV1.vote.getVotesByDni(req.body.dni).then((votes) => {
    const topicsIds = votes.map(v => v.topic.toString())
    if (votes.length >= config.cantVotesAvailable) {
      // return an error
      log('DNI ' + req.body.dni + ' has already reached the max votes available')
